...
window.onload = function () {
  const countdownElement = document.querySelector(".ul-countdown");
  if (!countdownElement) return;

  const days = parseInt(countdownElement.querySelector(".days").innerHTML, 10);
  const hours=parseInt(countdownElement.querySelector(".hours").innerHTML,10);
  const minutes=parseInt(countdownElement.querySelector(".minutes").innerHTML,10);
  const seconds = parseInt(countdownElement.querySelector(".seconds").innerHTML, 10);

  triggerCountdown(".ul-countdown", days, hours, minutes, seconds);
};
